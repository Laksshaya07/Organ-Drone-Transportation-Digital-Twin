...
class DroneTwin:
    ...
    def update_tick(self, time_step_sec=5.0):
        ...
        if self.status not in ["LANDED", "OFFLINE", "IDLE"]:
            ...
            wind_factor = (self.wind_speed / 10.0) * 0.05
            speed_factor = (self.speed / 70.0) * 0.08
            payload_factor = (payload_weight / 5.0) * 0.04
            
            # Base loss per virtual second: base consumption + factors
            base_rate = 0.035
            
            # Fast drain if battery failure injected
            if "Battery Failure" in self.faults:
                base_rate = 0.55  # Exceedingly high drain
    ...
    def auto_detect_failures(self):
        ...
        if self.battery < 20.0 and "Battery Failure" not in self.faults:
            # Avoid triggering Battery Failure for Normal Delivery unless critically dead (< 5.0%)
